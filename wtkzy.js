...
class CaishowWidget extends DmYY {
  ...
  /**
   * 重写父类的保存方法
   * 强制将数据同步保存到 ConfigManager (FileManager)
   */
  saveSettings(notify = true) {
    // 1. 关键修复：将 DmYY 内存中的 settings 写入到 settings.json 文件
    ConfigManager.save(this.settings);
    
    // 2. 为了保持兼容，同时也存一份到 Keychain (让父类处理)
    // 传入 false 防止父类重复弹窗
  ...
  async handleSpacingMenu(prefix) {
    ...
    // 如果不是第四套(第四套是列表，通常不需要指定第几条，只需过滤)，则添加序号选择
    if (prefix !== "s4") {
  ...
  async fetchData() {
    ...
    if (isLocked) {
      ...
      try {
        ...
        const c = ConfigManager.readCache("location_cache.json");
        if (c) location = c; else location.locality = "定位失败";
      }
    }
    ...
    try {
        ...
        const aqiUrl = `https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${location.latitude}&longitude=${location.longitude}&current=us_aqi&timezone=auto`;
        
        const reqW = new Request(weatherUrl); reqW.timeoutInterval = 8;
        const reqA = new Request(aqiUrl); reqA.timeoutInterval = 8;
        
        const [resW, resA] = await Promise.all([reqW.loadJSON(), reqA.loadJSON()]);
        
        weather = this.processOpenMeteo(resW, resA);
        if(weather.temp) ConfigManager.saveCache("weather_cache.json", weather);
    } catch (e) {
        const c = ConfigManager.readCache("weather_cache.json"); 
        if(c) weather = c;
    }

    let poetry = {};
    let isStyle2 = (this.settings.styleModel === "modern" || (args.widgetParameter && args.widgetParameter.indexOf("style2") > -1));
    if (!isStyle2) {
        try {
  ...
  processOpenMeteo(wData, aData) {
    if (!wData || !wData.current || !wData.daily) return {};
    let info = {};
    
    info.temp = Math.round(wData.current.apparent_temperature); 
    info.hum = wData.current.relative_humidity_2m + "%";
    info.ico = this.wmCodeToIcon(wData.current.weather_code, wData.current.is_day);
    
    if (wData.daily.uv_index_max && wData.daily.uv_index_max[0] !== undefined) {
        info.uv = Math.round(wData.daily.uv_index_max[0]);
    } else {
        info.uv = "-";
    }

    if (aData && aData.current && aData.current.us_aqi !== undefined) {
        info.aqi = this.airQuality(aData.current.us_aqi);
    } else {
        info.aqi = "-";
    }
    
    const t = wData.current.temperature_2m;
    if (t <= 5) info.comfort = "寒冷";
    else if (t <= 15) info.comfort = "凉爽";
    else if (t <= 26) info.comfort = "舒适";
    else if (t <= 32) info.comfort = "炎热";
    else info.comfort = "酷热";

    if (wData.daily.sunrise && wData.daily.sunrise[0]) info.sunrise = wData.daily.sunrise[0].split("T")[1];
    else info.sunrise = "--:--";
    
    if (wData.daily.sunset && wData.daily.sunset[0]) info.sunset = wData.daily.sunset[0].split("T")[1];
    else info.sunset = "--:--";

    if (wData.daily.temperature_2m_min && wData.daily.temperature_2m_min[0] !== undefined) {
        info.min = Math.round(wData.daily.temperature_2m_min[0]);
        info.max = Math.round(wData.daily.temperature_2m_max[0]);
    } else {
        info.min = "-"; info.max = "-";
    }

    const descMap = { 0: "晴朗", 1: "多云", 2: "多云", 3: "阴", 45: "雾", 51: "小雨", 61: "雨", 71: "雪", 95: "雷雨" };
    let dTxt = descMap[wData.current.weather_code] || "有雨";
    info.desc = `今日${dTxt}，最高${info.max}°，最低${info.min}°`;
    info.alertTitle = ""; 
    ...
    for (let i = 1; i < 14; i++) {
        try {
            ...
            info.future.push({
                ...
                ico: this.wmCodeToIcon(wData.daily.weather_code[i], 1)
            });
        } catch(e){}
    }
  ...
  wmCodeToIcon(code, isDay) {
    const map = {
        0: isDay ? "sun.max.fill" : "moon.fill",
        1: isDay ? "cloud.sun.fill" : "cloud.moon.fill",
        2: isDay ? "cloud.sun.fill" : "cloud.moon.fill",
        3: "cloud.fill",
        45: "cloud.fog.fill", 48: "cloud.fog.fill",
        51: "cloud.drizzle.fill", 53: "cloud.drizzle.fill", 55: "cloud.drizzle.fill",
        61: "cloud.rain.fill", 63: "cloud.rain.fill", 65: "cloud.rain.fill",
        80: "cloud.rain.fill", 81: "cloud.rain.fill", 82: "cloud.rain.fill",
        71: "cloud.snow.fill", 73: "cloud.snow.fill", 75: "cloud.snow.fill",
  ...
  getNextHolidays() {
    ...
    // 【核心修复】从去年(currentYear-1)开始循环，以捕捉年初时的上一农历年生日
    for (let y = currentYear - 1; y <= currentYear + 1; y++) {
      
      // 添加公历固定节日 (只添加今明两年的，去年的没必要算)
      if (y >= currentYear) {
      ...
      // 添加用户生日 (关键逻辑：无论哪一年都计算，后续靠filter过滤掉过去的)
      ...
      if (bData) {
          ...
          for (let line of lines) {
              ...
              try {
                  if (type === "lunar") {
                      // 这里会计算 2025年的农历11月14日 -> 得到 2026年1月2日
    ...
    .filter(h => h && !isNaN(h.days) && h.days >= 0) // 过滤掉已经过去的
