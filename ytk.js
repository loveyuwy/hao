...
class CaishowWidget extends DmYY {
  ...
  Run() {
    if (config.runsInApp) {
      ...
      this.registerAction("检查更新", async () => { await this.updateScript(false) }, { name: 'cloud.fill', color: '#007aff', desc: `当前版本 v${ScriptVersion}` });
  ...
  async updateScript(isSilent = false) {
    const url = "https://raw.githubusercontent.com/loveyuwy/hao/refs/heads/main/ytk.js?ts=" + Date.now();
    const a = new Alert();
    try {
        ...
        console.log(`[更新检测] 本地: ${ScriptVersion}, 远程: ${remoteVersion}, 静默: ${isSilent}`);

        if (!remoteVersion) {
            if (isSilent) return;
            a.title = "⚠️ 无法检测远程版本";
            a.message = "无法解析远程文件版本号，是否强制覆盖？";
            a.addAction("强制更新"); a.addCancelAction("取消");
            if (await a.presentAlert() === 0) await this.doUpdate(html);
            return;
        }
        ...
        if (compareVersion(remoteVersion, ScriptVersion) > 0) {
            ...
            a.title = "✅ 已是最新版本";
            a.message = `远程版本: v${remoteVersion}\n当前版本: v${ScriptVersion}\n\n您的版本是最新的（或比远程版本更高）。`;
            a.addAction("好的");
