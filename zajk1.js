...
(async () => {
  ...
  if (typeof $argument !== "undefined" && $argument) {
    console.log(`[环境检查] 成功获取到 $argument: ${$argument}，开始解析...`);
    
    // 去除 Loon 参数格式可能带有的前后中括号 []
    let argStr = $argument.replace(/^\[|\]$/g, '');
    
    // 同时兼容 Surge 的 "#" 分割和 Loon 的 "," 分割
    tokens = argStr.split(/[#,]/)
  ...
  if (tokens.length === 0) {
    console.log(`[配置错误] 未检测到任何 Token，请在模块/重写/插件设置中填入参数。`);
    $.notify("🏥 众安健康", "❌ 配置错误", "请先在配置中填入至少一个 Token");
    $.done();
    return;
  }
