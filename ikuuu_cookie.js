/*
 * iKuuu 自动获取 Cookie (独立版)
 * 兼容：Surge, Loon, Quantumult X
 * 使用方法：
 * 1. 配置好脚本和 MITM
 * 2. 浏览器访问 https://ikuuu.de/user (需登录)
 * 3. 弹出通知后即可禁用此脚本
 */

const $ = new Env("iKuuu获取Cookie");
...
function GetCookie() {
    ...
    if (typeof $request !== "undefined" && $request.headers) {
        ...
        if (cookie) {
            // 写入持久化存储，Key 为 "ikuuu_cookie" (与签到脚本一致)
            const saved = $.setdata(cookie, "ikuuu_cookie");
            
            if (saved) {
                // 成功抓取，发送通知
                $.msg($.name, "获取成功 🎉", "Cookie 已更新，签到脚本可正常使用了");
                console.log("✅ Cookie 已保存: " + cookie);
            } else {
                $.msg($.name, "获取失败", "写入数据失败");
            }
        } else {
            console.log("⚠️ 请求头中未包含 Cookie");
        }
    } else {
        console.log("❌ 非请求模式触发，请检查配置");
    }
}
...
})){t.headers&&(delete t.headers["Content-Type"],delete t.headers["Content-Length"]),this.isSurge()||this.isLoon()?(this.isSurge()&&this.isNeedRewrite&&(t.headers=t.headers||{},Object.assign(t.headers,{"X-Surge-Skip-Scripting":!1})),$httpClient.get(t,(t,s,i)=>{!t&&s&&(s.body=i,s.statusCode=s.status),e(t,s,i)})):this.isQuanX()?(this.isNeedRewrite&&(t.opts=t.opts||{},Object.assign(t.opts,{hints:!1})),$task.fetch(t).then(t=>{const{statusCode:s,statusCode:i,headers:r,body:o}=t;e(null,{status:s,statusCode:i,headers:r,body:o},o)},t=>e(t))):this.isNode()&&(this.initGotEnv(t),this.got(t).on("redirect",(t,e)=>{try{if(t.headers["set-cookie"]){const s=t.headers["set-cookie"].map(this.cktough.Cookie.parse).toString();s&&this.ckjar.setCookieSync(s,null),e.cookieJar=this.ckjar}}catch(t){this.logErr(t)}}).then(t=>{const{statusCode:s,statusCode:i,headers:r,body:o}=t;e(null,{status:s,statusCode:i,headers:r,body:o},o)},t=>{const{message:s,response:i}=t;e(s,i,i&&i.body)}))}post(t,e=(()=>{
